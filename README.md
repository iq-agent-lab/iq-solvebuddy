...
**현재 버전: v0.9.0** — 멀티 플랫폼 기반 (Phase 1 — LeetCode만 실제 동작, Programmers / AtCoder / Codeforces / BOJ는 Phase 2-5에서 추가). 자세한 진척은 [로드맵](#로드맵) 참조.
...
- **macOS native notification** — 업로드 완료 시 `✓ leetbuddy 업로드 완료` 시스템 알림 (첫 사용 시 권한 1회 요청)
...
### v1.0 (다음 후보) — Phase 2: 프로그래머스

- [ ] **프로그래머스 지원** — HTML scraping (공식 API 없음), 한국어 사이트라 번역 prompt 살짝 조정. SQL 문제도 동일 흐름
- [ ] 도구 이름 변경 검토 — `iq-leetbuddy` → 더 범용적 이름 (`iq-solvebuddy` 등). 기존 brand 보존 vs 정확성 trade-off
