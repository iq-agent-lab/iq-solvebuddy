...
**LeetCode + 프로그래머스 + AtCoder** 문제 풀이를 **한국어로 정리**해 보여주고, 통과한 코드에 **AI 회고**를 붙여 **GitHub에 자동 정리**하는 데스크톱 에이전트.

iq-agent-lab 행성 중 하나. 매일 문제 풀이를 *기록 가능한 학습 자산*으로 바꾸는 것이 이 행성의 일.

**현재 버전: v1.1.0** — LeetCode + 프로그래머스 + AtCoder (Phase 1-3 완료). Codeforces / BOJ는 Phase 4-5에서 추가 예정. 자세한 진척은 [로드맵](#로드맵) 참조.

> 플랫폼별 statement 정책: **LeetCode/AtCoder**는 영어 원문 → 한국어 번역. **프로그래머스**는 한국어 원문 → 정리만 (HTML→마크다운). **AtCoder**는 영어 우선 + 일본어 fallback.
...
### v1.2+ (다음 후보) — Phase 4-5

- [ ] **Codeforces (Phase 4)** — public API(`/api/problemset.problems`) + statement HTML scraping. rating 기반 색깔 난이도 (800~3500)
- [ ] **백준 BOJ (Phase 5)** — HTML scraping + solved.ac API 난이도 (Bronze V ~ Ruby I)
