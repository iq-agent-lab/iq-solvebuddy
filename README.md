...
**LeetCode + 프로그래머스** 문제 풀이를 **한국어로 정리**해 보여주고, 통과한 코드에 **AI 회고**를 붙여 **GitHub에 자동 정리**하는 데스크톱 에이전트.

iq-agent-lab 행성 중 하나. 매일 문제 풀이를 *기록 가능한 학습 자산*으로 바꾸는 것이 이 행성의 일.

**현재 버전: v1.0.0** — LeetCode + 프로그래머스 (Phase 1-2 완료). AtCoder / Codeforces / BOJ는 Phase 3-5에서 추가 예정. 자세한 진척은 [로드맵](#로드맵) 참조.

> 프로그래머스는 한국어 원문이라 **번역하지 않고 정리만** — HTML을 깔끔한 마크다운으로 변환 + 메타 헤더 추가. LeetCode는 기존대로 영어→한국어 번역.
...
**프로그래머스 (Programmers)**: 한국어 원문이라 **정리 모드** — HTML을 깔끔한 마크다운으로 변환 + 메타 헤더 추가. 번역 안 함. SQL 문제도 동일 흐름. Lv 0-2 비로그인 접근 OK, Lv 3+ 로그인 필요 문제는 Phase 2.5(v1.1+)에서 임베드 세션 활용 예정.
...
### v1.1+ (다음 후보) — Phase 2.5

- [ ] **프로그래머스 임베드 세션 활용** — Lv 3+ 로그인 필요 문제도 가져오기. 임베드 LeetCode와 같은 패턴 (`persist:programmers`)
- [ ] **submission 자동 fetch (Programmers)** — 사용자 마지막 통과 코드 자동 가져오기
- [x] **도구 이름 변경** — `iq-leetbuddy` → `iq-solvebuddy` (v1.0+ 멀티 플랫폼이 정확한 이름). GitHub repo rename + Releases / 자동 업데이트 URL 호환. 사용자 데이터는 그대로 유지 (`app.setName('iq-leetbuddy')` + localStorage key prefix 유지)

### v1.2+ — Phase 3-5

- [ ] **AtCoder** — HTML scraping, 영어/일본어 statement
- [ ] **Codeforces** — public API + statement scraping, rating 기반 표시
- [ ] **백준 (BOJ)** — HTML + solved.ac 난이도 (Bronze ~ Ruby)
