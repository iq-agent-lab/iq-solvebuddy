...
**현재 버전: v1.12.1** — 🎨 **Obsidian 무드 디자인 개선**. 광물 결정 아이콘, 은은한 글로우, glass surface, dark/light 테마 시각 보강. v1.12의 4개 플랫폼 풀 패리티, 통과 무결성, Gist sync, CodeMirror 검색/fold, 임베드 윈도우 추상화는 유지. 자세한 진척은 [로드맵](#로드맵) 참조.
...
| Mac 종류 | 파일 |
|---|---|
| Apple Silicon (M1, M2, M3, M4) | `Solve.Buddy-{version}-arm64-mac.zip` |
| Intel Mac | `Solve.Buddy-{version}-mac.zip` |

> v1.7+ 부터 파일명에 점(`.`) 포함 — electron-builder가 공백을 점으로 sanitize. zip 안의 `.app`은 `Solve Buddy.app` (공백 포함) 또는 `Solve.Buddy.app` 둘 중 하나로 풀림.
...
![Solve Buddy은(는) 손상되었기 때문에 열 수 없습니다 경고](docs/images/gatekeeper-warning.png)

다운로드한 zip을 풀어서 `.app` 파일을 더블클릭하면 macOS가 위 같은 경고를 띄움:

> **'Solve Buddy'은(는) 손상되었기 때문에 열 수 없습니다. 해당 항목을 휴지통으로 이동해야 합니다.**

**진짜 손상된 게 아니야.** Chrome으로 다운받은 unsigned 앱에 macOS가 `com.apple.quarantine`이라는 "출처 모름" 꼬리표를 붙이고, Gatekeeper가 그걸 보고 *손상이라고 거짓말*하면서 차단하는 거. Apple Developer cert가 있는 *서명된* 앱이면 안 뜨는데, Solve Buddy는 개인 도구라 cert 없음.
...
```bash
cd /tmp && \
unzip -o ~/Downloads/Solve.Buddy-*-mac.zip && \
APP=$(ls -d Solve*.app | head -1) && \
xattr -cr "$APP" && \
rm -rf '/Applications/Solve Buddy.app' '/Applications/Solve.Buddy.app' '/Applications/iq-solvebuddy.app' '/Applications/iq-leetbuddy.app' && \
mv "$APP" /Applications/ && \
open "/Applications/$APP" && \
rm -f ~/Downloads/Solve.Buddy-*-mac.zip
```

> **옛 버전 사용자** (v0.x `iq-leetbuddy` / v1.0~v1.6 `iq-solvebuddy`): 위 명령이 `/Applications/`에 남아있는 옛 .app 4가지(`iq-leetbuddy.app`, `iq-solvebuddy.app`, `Solve Buddy.app`, `Solve.Buddy.app`) 모두 한 번에 정리. 사용자 데이터(API 키 / 통계 / draft)는 그대로 살아 있음 — userData 폴더 `iq-leetbuddy`는 모든 버전 호환 유지.
...
|---|---|
| `cd /tmp` | 작업 디렉토리로 이동 |
| `unzip -o ~/Downloads/Solve.Buddy-*-mac.zip` | Downloads의 zip 풀기 (Apple Silicon/Intel 자동 매칭) |
| `APP=$(ls -d Solve*.app \| head -1)` | zip 안 .app 이름 동적 캡처 (`Solve Buddy.app` / `Solve.Buddy.app` 둘 다 대응) |
| `xattr -cr "$APP"` | quarantine 꼬리표 제거 (이게 핵심) |
| `rm -rf /Applications/...` (4개) | 옛 모든 버전 .app 제거 (이름 다르므로 `mv`로 덮어쓰기 안 됨) |
| `mv "$APP" /Applications/` | Applications 폴더로 옮기기 |
| `open "/Applications/$APP"` | 실행 |
| `rm -f ~/Downloads/Solve.Buddy-*-mac.zip` | 다운로드 정리 — 다음 업데이트 시 glob 혼란 회피 |
...
3. **Owner / Repository**: 본인 GitHub 사용자명 + 풀이 레포 이름 (예: `e9ua1` / `algorithm-solutions`)
4. **"GitHub 연결 확인"** 버튼 → 토큰/레포 한 번에 진단. 레포 없으면 그 자리에서 **"지금 만들기"** 클릭
5. (선택) **"레포 없을 때 자동 생성"** 토글 켜기

저장 후 메인 화면에서 **4개 플랫폼 URL** 중 하나 던지면 끝 (LeetCode / 프로그래머스 / AtCoder / Codeforces).
...
- LeetCode/AtCoder/CF 문제는 영어라 한국어 학습자에게 *이해 자체가 비용*이고,
- 코드만 commit되면 **나중에 그 풀이를 다시 꺼냈을 때 맥락을 잃는다.**
- 회고 없이 정답 코드만 쌓으면, 100문제 풀어도 95문제는 잊는다.

Solve Buddy는 *commit*이 아니라 *학습*을 자동화한다.
...
> userData 폴더가 `iq-leetbuddy`인 이유: v0.x 사용자의 데이터(API 키 / 캐시 / 풀이 통계) 손실 방지를 위해 폴더명은 호환성 유지. 앱 표시 이름만 v0.x `iq-leetbuddy` → v1.0+ `iq-solvebuddy` → v1.7+ `Solve Buddy`로 진화.

> v1.6.2+ 자동 cache invalidation — 옛 추출 로직으로 cache된 결과(`?` / `Lv ?`)는 다음 fetch 시 자동 refresh. 수동 삭제 불필요한 경우 대부분.
...
### v1.4.x (완료) — Phase 3.5: AtCoder 임베드 + submission 자동 fetch

- [x] **AtCoder 임베드 윈도우** — `persist:atcoder` 영속 세션. 헤더 AtCoder 버튼 클릭 시 임베드 열림 (외부 브라우저 → 임베드). INJECT_SCRIPT chip 버튼 + console-message sentinel + URL pull/push (LeetCode 패턴 평행)
- [x] **AtCoder submission 자동 fetch** — `/submissions/me?f.Task={taskId}&f.Status=AC` HTML scraping. 임베드 세션 cookies 활용. 가장 최근 AC submission ID → `<pre id="submission-code">` 코드 추출
- [x] **AtCoder lang 매핑** — "C++ 23 (gcc 12.2)" / "Python (3.11.4)" 등 다양한 표기 → 우리 langSlug 매핑 (substring 검사)
...
### "Solve Buddy은(는) 손상되었기 때문에 열 수 없습니다"
...
```bash
xattr -cr '/Applications/Solve Buddy.app'
# .app 이름이 점 형태로 풀린 경우:
xattr -cr '/Applications/Solve.Buddy.app'
```
...
`.app`은 macOS에서 디렉토리(번들). `mv -f`는 *파일* 덮어쓰기는 되지만 디렉토리는 target이 비어있어야. 위 설치 명령의 `rm -rf ... && mv ...` 패턴이 이걸 해결 (옛 4가지 .app 이름 모두 정리).
