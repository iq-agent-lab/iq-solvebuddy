...
| 임베드 윈도우 추상화 (v1.12+) | 유지 | 4개 플랫폼 공통 로직(persist 윈도우/INJECT_SCRIPT chip/URL pull-push) → `src/main/embedWindow.ts`의 `EmbedController` factory. 4개 platform-specific config 객체 + thin wrapper 함수. main/index.ts 1100 → 580 lines 축소. LeetCode lang hint는 `extraInjectScriptJs` option으로 통합. 미래 BOJ 등 5번째 플랫폼 추가가 config 한 개로 |
| Gist 백업 디바이스 sync (v1.12+) | 유지 | localStorage solutions → private gist push/pull merge. stats 모달 "☁ Gist 백업" 버튼 한 번 클릭으로 양방향 sync. conflict resolution: savedAt 기준 newest wins. PAT에 gist scope 필요 (settings 안내 + URL pre-check). gistId localStorage 저장 → 다음 sync 직접 조회 |
| CodeMirror addons (v1.12+) | 유지 | search (Cmd+F dialog) / foldgutter (큰 블록 접기) / jump-to-line / lineWrapping toggle. 사용자 큰 코드(AC/CF 50+줄) 작업 시 가치. lineWrapping은 settings 토글 (default OFF — 짧은 LC 풀이는 가로 스크롤 선호) |
...
| Accepted 사전 확인 = override 허용 (차단 X), LC/AC/CF 지원 (v1.10+) | 유지 | 본 도구 핵심 가치는 통과한 풀이 학습 자산화. 단 완전 차단은 다른 OJ/오프라인/세션 만료 등 예외 케이스 막음 → confirm + override. `hasAcceptedSubmission` null(미로그인/API fail)은 silent skip. 모달 "다시 묻지 않음" 체크박스로 토글 OFF. **v1.10+** payload union으로 LC/AC/CF 3개 플랫폼 통합 (PG는 AC 개념 없어 사전 확인 미지원). 각 플랫폼별 fetch: LC graphql / AC submissions/me HTML / CF contest/N/my HTML — 이미 submission fetch에서 활용하던 endpoint 재사용 |
...
| KaTeX = nonStandard 모드 | 유지 | 기본 inline regex가 closing `$` 뒤에 공백/구두점/줄끝만 허용 → 한국어 (`$1$부터`) 매칭 실패. `nonStandard: true`로 boundary 요구 제거 — 한국어 문맥에서 수식 정상 렌더. throwOnError:false 그대로 |
| CF rating = title attr fallback + lenient pattern | 유지 | 기본 `*1500` 패턴 외에도 `★1500` / title="Difficulty: 1500" / pure digit + difficulty 힌트 등 다중 fallback. 페이지 구조 변경 / 일부 페이지 차이 대응 |
| AtCoder heading = 제목만 (taskId 제거) | 유지 | 사용자 피드백: "abc458_a." prefix가 가독성 떨어짐. h1은 title만, taskId는 메타 blockquote(`> ${difficulty} · ${contestId} / ${taskId} · [원문]`)에서 별도 표시 |
| 플랫폼 구분 = 색깔 abbr badge | 유지 | recent chips + stats dashboard에 LC/PG/AC/CF/BJ 약자 badge 추가. 플랫폼별 색깔(LeetCode 주황 / Programmers 녹색 / AtCoder 검정 / Codeforces 파랑). RecentItem에 platform 필드 추가, legacy 데이터는 'LeetCode' fallback. inputUrl 필드 추가로 chip 재클릭 시 원본 URL 복원 (parseProblemInput dispatch 정확) |
| 캐시 stale auto-invalidation | 유지 | readTranslationCache가 cached difficulty가 '?' / 'Lv ?' / '?점' 이면 cache miss로 처리. 옛 fix 이전 캐시된 결과가 자동 refresh — 사용자가 수동 캐시 삭제 안 해도 됨 |
| 헤더 타이틀 = "Solve Buddy" (v1.6.3+) | 유지 | "leetbuddy" → "Solve Buddy" (멀티 플랫폼이라 LC 한정 이름 부적절). 코랄 행성 + Fraunces 폰트 시그니처는 유지 (사용자 "살아있는 느낌" 명시 요청). LeetCode-specific 카피 텍스트(step-2 hint, step-3 hint, fetch 버튼 라벨)는 모두 platform-agnostic으로 변경 |
| productName "Solve Buddy" (v1.7.0+) | 유지 | electron-builder productName 변경: `iq-solvebuddy` → `Solve Buddy`. .app 파일명 / window title / tray / dock 다 변경. **userData 폴더는 그대로 'iq-leetbuddy'** (app.setName 명시) — 사용자 데이터 손실 X. zip 파일명은 `Solve Buddy-{ver}-mac.zip` (공백 포함) → CLI에서 quote 필요 |
| CF rating fallback = Division + index | 유지 | 페이지에서 `*NNN` rating 못 가져오면 contest 이름 (`Codeforces Round 945 (Div. 2)`)에서 Division 추출 후 `Div.2 C` 형식으로 표시. 사용자 제공 CF 난이도 가이드 (Div.4 C는 단순 그리디 / Div.2 C는 알고리즘 체급 등) 반영 |
| PG level = 임베드 JS fallback | 유지 | 정적 HTML(cookies fetch)에서 `.level-active` 못 찾으면 임베드 윈도우의 `webContents.executeJavaScript`로 client-rendered DOM 추출. 같은 lessonId 페이지에 있어야 함. `setProgrammersWindowGetterForLevel` setter로 main → service 주입 |
| 통계 dashboard 플랫폼 탭 | 유지 | 4개 플랫폼이 난이도 기준 다름 (LC: Easy/Med/Hard, PG: Lv 1~5, AC: 점수 구간, CF: ★rating 구간). 전체 + 4개 platform 탭으로 분리. 각 탭마다 `difficultyBins()` 헬퍼가 platform-specific bin 생성. 'all' 탭은 LC 기준 (가장 익숙) |
| PG level = 별 아이콘 count 우선 | 유지 | 프로그래머스 UI는 별 N개로 level 표시 — `.level-active` count가 가장 정확한 source. 컨테이너(.level/.challenge-level) 안 / page-wide 둘 다 검사. 못 찾으면 텍스트 selector → 클래스명 → inline JSON 순 fallback |
| CF rating = 3-tier fallback | 유지 | 1) `.tag-box` cheerio 순회 (text + title attr) → 2) raw HTML regex (`*NNN` near tag-box class) → 3) page-wide `*NNN` 단독 패턴 (CF rating 범위 800~3500 가드). 페이지 구조 변경 / nested anchor 모두 대응 |
...
| AtCoder difficulty rating = kenkoooo.com API + 색깔 체계 | 유지 | AtCoder 페이지엔 점수만 있음. AtCoder Problems API가 IRT 기반 difficulty 제공. **30MB+ JSON** (gzip ~5-10MB) — 24h TTL disk + 메모리 캐시 + 부팅 prewarm. v1.9+ AtCoder Problems 표준 색깔 체계 적용 (회색<400 / 갈색<800 / 녹색<1200 / 청색<1600 / 황색<2000 / 주황<2400 / 적색<2800 / 레전드). 표기: "300점 · 🟢 녹색 (920)" / 음수 rating은 "100점 · ⚫ 회색" |
| PG level = 3-tier fallback | 유지 | 1) 정적 HTML cheerio 추출 → 2) 임베드 윈도우 executeJavaScript (빠름, 임베드 있을 때만) → 3) hidden BrowserWindow + executeJsOnPage (~3초, robust). PG SPA는 hydration 후에만 `.level-active`가 채워짐 → 임베드 없으면 hidden 윈도우가 정확함. 캐시되면 다음부터 fast |
| browserFetch executeJsOnPage | 유지 | fetchHtmlViaBrowser는 HTML만 반환 → executeJsOnPage 추가 (URL 로드 + JS 실행 + return). SPA hydration 대기 1.5s. PG level 추출에 활용. 향후 다른 client-rendered 데이터에도 |
| 풀이 검색 = title/slug/lang/번호 부분일치 | 유지 | stats 모달에 search box. 검색어 있으면 전체 풀이에서 필터 (최근 10개 제한 X), 없으면 기존 동작 (최근 10개). statsSearchQuery state + renderStatsDashboard 분기 |
